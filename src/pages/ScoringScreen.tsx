...
const ScoringScreen: React.FC = () => {
  ...
  const fielders = bowlingTeam.players.filter(player => player.id !== innings.currentBowler);
  
  const isTargetSet = currentMatch.innings.length > 1 && currentMatch.currentInnings === 1;
  const target = isTargetSet ? currentMatch.innings[0].totalRuns + 1 : undefined;
  const runsLeft = target ? target - innings.totalRuns : undefined;
  const ballsLeft = (currentMatch.totalOvers * 6) - (innings.currentOver * 6 + innings.currentBall);
  ...
  const isInningsCompleted = isLastWicket || isAllOversCompleted || 
                            (isTargetSet && innings.totalRuns >= target - 1);
  ...
  if (innings.overs.length > 0) {
    ...
    currentOver.balls.forEach((ball) => {
      if (ball.type === 'wicket') {
        currentOverBalls.push(<span key={currentOverBalls.length} className="text-error font-bold">W</span>);
      } else if (ball.type === 'run') {
        currentOverBalls.push(<span key={currentOverBalls.length}>{ball.value}</span>);
      } else if (ball.type === 'extra') {
        if (ball.extraType === 'wide') {
          currentOverBalls.push(<span key={currentOverBalls.length} className="text-warning">WD</span>);
        } else if (ball.extraType === 'noBall') {
          currentOverBalls.push(<span key={currentOverBalls.length} className="text-warning">NB</span>);
        }
      }
    });
  }
  ...
  const handleWicketConfirm = () => {
    ...
    handleScoringAction({
      type: 'wicket',
      value: 0,
      wicketType: wicketDetails.type as any,
      fielder: wicketDetails.fielder || undefined,
      batsmanOut: innings.striker,
      ...
      <div className="max-w-lg mx-auto pb-10">
        ...
        <Card className="mb-4">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-lg font-medium text-white/70">{battingTeam.name}</h2>
            ...
            <div className="bg-primary/10 p-3 rounded-lg mb-3">
              <div className="flex justify-between">
                ...
                <div>
                  ...
                  <div className="font-bold">{runsLeft} from {Math.ceil(ballsLeft / 6)} overs</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-accent/10 p-3 rounded-lg">
              ...
              <div className="text-sm text-white/70">
                {nonStriker.batting?.runs || 0} ({nonStriker.batting?.balls || 0})
              </div>
            </div>
          </div>
                ...
                <div className="inline-flex space-x-2 ml-2">
                  {currentOverBalls.length > 0 ? (
                    currentOverBalls.map((ball, i) => (
                      <span key={i} className="w-6 h-6 flex items-center justify-center bg-surface-light rounded-full">
                        {ball}
                      </span>
                    ))
                  ) : (
            ...
            <div className="text-center">
              <div className="bg-primary/10 p-4 rounded-lg mb-4">
                ...
                <p className="text-white/70 mb-4">
                  {isLastWicket ? 'All out' : 
                   isAllOversCompleted ? 'All overs completed' : 
                   'Target achieved'}
                </p>
                ...
                <Button
                  ...
                  variant="primary"
                >
                  {currentMatch.currentInnings === 0 ? 'Start Second Innings' : 'View Match Summary'}
                </Button>
              </div>
            </div>
          ) : (
          ...
          >
            ...
            <div className="mb-4">
              <Select
                ...
                onChange={(value) => setWicketDetails({ ...wicketDetails, type: value })}
                fullWidth
              />
            </div>
            
            {(wicketDetails.type === 'caught' || wicketDetails.type === 'runOut' || wicketDetails.type === 'stumped') && (
              <div className="mb-4">
                <Select
                  label="Fielder"
                  ...
                  onChange={(value) => setWicketDetails({ ...wicketDetails, fielder: value })}
                  fullWidth
                />
              </div>
            )}
            ...
            <div className="mb-4">
              <Select
                ...
                onChange={(value) => setWicketDetails({ ...wicketDetails, newBatsman: value })}
                fullWidth
                error={!wicketDetails.newBatsman ? 'Please select the next batsman' : undefined}
              />
            </div>
          ...
          >
            ...
            <div className="mb-4">
              <Select
                ...
                fullWidth
                error={!nextBowler ? 'Please select the next bowler' : undefined}
              />
            </div>
